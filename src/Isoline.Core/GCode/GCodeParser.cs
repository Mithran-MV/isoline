	...
	public static class GCodeParser
	{
		...
		static void Parse(string line, int lineNumber)
		{
			...
			for (int i = 0; i < Words.Count; i++)
			{
				...
				if (IgnoreAxes.Contains(Words[i].Command) && GCodeParserOptions.Current.IgnoreAdditionalAxes)
				{
					Words.RemoveAt(i--);
					continue;
				}
			...
			for (int i = 0; i < Words.Count; i++)
			{
				...
				if (Words[i].Command == 'G' && !MotionCommands.Contains(Words[i].Parameter))
				{
					...
					if (param == 94)
					{
						// units per minute feed. This is the mode the parser already assumes,
						// so the word is simply consumed. Upstream warned about it, which
						// meant almost every file exported by Fusion 360 or KiCad opened with
						// a warning that meant nothing.
						Words.RemoveAt(i);
						i--;
						continue;
					}
					if (param == 93)
					{
						Warnings.Add($"inverse time feed (G93) is not supported, feed rates will be wrong. (line {lineNumber})");
						Words.RemoveAt(i);
						i--;
						continue;
					}
