	...
	public partial class GrblSettingsWindow : Window
	{
		...
		private async void ButtonApply_Click(object sender, RoutedEventArgs e)
		{
			...
			foreach (KeyValuePair<int, double> kvp in CurrentSettings)
			{
				...
				{
					MessageBox.Show($"Value \"{SettingsBoxes[kvp.Key].Text}\" is invalid for Setting \"{GrblCodeTranslator.Settings[kvp.Key].Item1}\"");
					return;
				}
		...
		private void ButtonExport_Click(object sender, RoutedEventArgs e)
		{
			...
			foreach (KeyValuePair<int, double> kvp in this.CurrentSettings)
			{
				...
				{
					MessageBox.Show($"Value \"{this.SettingsBoxes[kvp.Key].Text}\" is invalid for Setting \"{GrblCodeTranslator.Settings[kvp.Key].Item1}\"");
					return;
				}
